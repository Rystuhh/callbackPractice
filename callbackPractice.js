...
function first(arrayOfNames, cb) {
  var firstNameInArray = arrayOfNames[0];
  cb(firstNameInArray);
}
...
function last(arrayOfNames, cb) {
  var lastNameInArray = arrayOfNames[arrayOfNames.length - 1];
  cb(lastNameInArray);
}
...
function multiply(firstNumber, secondNumber, cb) {
  var answerFromAbove = firstNumber * secondNumber;
  cb(answerFromAbove);
}
...
function getUserById(arr, obj, cb) {
  for (var i = 0; i < arr.length - 1; i++) {
    if (arr[i] === obj) {
    }
    return cb(arr[i]);
